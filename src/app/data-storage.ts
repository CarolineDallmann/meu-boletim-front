...
export class DataStoreService {
    ...
    public usuarioConectado = new BehaviorSubject<PessoaLocalStorage | undefined>(undefined)
    ...
    updateUsuario(usuario: PessoaLocalStorage | undefined) {
        ...
        if (usuario) {
            ...
            if ([TipoPessoa.ALUNO, TipoPessoa.RESPONSAVEL].includes(usuario.tipoPessoa)) {
                this.router.navigate(['boletim'])
            } else {
                setTimeout(() => {
                    if (this.router.url.includes('login') || this.router.url.includes('boletim')) {
